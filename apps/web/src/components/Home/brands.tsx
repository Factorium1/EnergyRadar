...
const HomeBrands = ({
  ...
  return (
    <div className="mx-auto w-full max-w-7xl">
      <h2 className="mb-5 text-2xl text-gray-900 font-semibold">Beliebte Marken</h2>

      {isError && error && (
        <span className="mb-5 block font-semibold text-red-500">{error.message}</span>
      )}
          ...
          data.map((brand) => (
            ...
            >
              <div className="mx-auto mb-2.5 flex h-12 w-12 items-center justify-center rounded-[10px] bg-[#1a1f71]">
                <span className="text-xl font-black text-white">{brand.name.charAt(0)}</span>
              </div>
              <div className="text-xs font-extrabold text-gray-900">{brand.name}</div>
            </Link>
          ))}
      </div>
    </div>
