...
async function getBasicData(): Promise<ScraperProduct[]> {
  ...
  $("div.product-tile[data-test='product-tile']").each((_i, el) => {
    ...
    const brandName = $el.find("[data-test='product-tile__brandname'] p").text().trim()
    ...
    const pricePerLiterRaw = $el.find("[data-test='product-tile__comparison-price'] p").text()
    const pricePerLiter = parseGermanNumber(pricePerLiterRaw)

    const sizeRaw = $el.find("[data-test='product-tile__unit-of-measurement'] p").text()
