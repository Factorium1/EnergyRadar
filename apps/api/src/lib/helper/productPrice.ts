 ...
 */
export function getProductsWithPriceChange(products: ReturnType<typeof getProductPriceMedian>) {
  return products.map((product) => ({
    ...product,
...
export function buildPriceTimeline(history: PriceHistoryRow[], days = 30) {
  ...
  while (cursor < sorted.length && sorted[cursor]!.recordedAt.getTime() < start.getTime()) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    while (cursor < sorted.length && sorted[cursor]!.recordedAt.getTime() < dayEnd) {
      remember(sorted[cursor]!)
      cursor++
    }
...
export function sortProductsByCheapest(products: ReturnType<typeof getProductsWithPriceChange>) {
